...
This is only possible to do reliably via some kind of rig control server.
...
When using FT8 or FT4, I notice that sometimes my rig frequency changes when going from receive to transmit.  Is this a problem?::

No. If you are using *Split*, and your Tx audio frequency (red goalpost on the waterfall) is outside the range 1500-2000Hz, your rig
will move frequency. This is because the chosen audio frequency could generate in-band harmonics if the audio level
is too high, or may be outside the typical bandwidth of an ssb filter.  The generated audio tone is adjusted accordingly, and the rig
frequency is changed to compensate for this.
...
I am running _WSJT-X_ on Linux using a KDE desktop. Why does the *Configurations* menu misbehave?::
