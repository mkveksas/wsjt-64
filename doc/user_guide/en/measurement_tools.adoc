...
shape of your receiver's passband. Tune to a quiet frequency with no
signals or birdies. With _WSJT-X_ running in one of the slow modes and
*Monitor* active, make sure that *Flatten* is checked on the *Wide
Graph* and then select *Measure reference spectrum* from the *Tools*
menu.  Wait for about a minute and then click *Stop*.  A file named
`refspec.dat` will appear in your log directory.  When you check *Ref
Spec* on the *Wide Graph*, the recorded reference spectrum will then
be used to flatten your overall effective passband.
