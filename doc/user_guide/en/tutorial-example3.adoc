...
Activity/Rx Frequency* windows should look something like the
...
main window will illuminate three times, once for each pass.
The first decoding attempt in each pass is done at the selected Rx frequency,
...
of decodes is shown in a box at the bottom of the main window.  When
this file was recorded HA5WA was finishing a QSO with K1JT, and his 73
message is shown in red because it is addressed to *My Call* (in this
case K1JT).  By default, lines containing CQ are highlighted in green,
and lines with *My Call* (K1JT) in red. Notice that K1JT now has two
additional callers; HA0DU and EA3AGB.
