...
subroutine osd174var(llr,apmask,ndeep,decoded,cw,nhardmin,dmin,nthr)
!
...
interface
  subroutine boxitvar(indexes,fp,np,reset,e2,ntau,npindex,i1,i2)
    integer indexes(:,:),fp(0:),np(:),ntau
    integer*1 e2(1:ntau)
    integer npindex,i1,i2
    logical reset
  end subroutine boxitvar
  subroutine fetchitvar(indexes,fp,np,reset,e2,ntau,i1,i2,nthr)
    integer indexes(:,:),fp(0:),np(:),ntau
    integer*1 e2(1:ntau)
    integer i1,i2,nthr
    logical reset
  end subroutine fetchitvar
end interface
...
call mrbencodevar(m0,c0,g2,N,K)
nxor=ieor(c0,hdec)
nhardmin=sum(nxor)
dmin=sum(nxor*absrx)
...
do iorder=1,nord
   ...
   do while(iflag .ge.0)
      ...
      do n1=iflag,iend,-1
         ...
         me=ieor(m0,mi)
         if(n1.eq.iflag) then
            call mrbencodevar(me,ce,g2,N,K)
...
! when the last pattern with weight iorder has been generated.
      call nextpatvar(misub,k,iorder,iflag)
   enddo
enddo
...
if(npre2.eq.1) then
   ...
   do i1=K,1,-1
      do i2=i1-1,1,-1
         ...
         call boxitvar(indexes,fp,np,reset,mi(1:ntau),ntau,ntotal,i1,i2)
      enddo
   enddo
...
! Now run through again and do the second pre-processing rule
   ...
   do while(iflag .ge.0)
      ...
      do i2=0,ntau
         ...
         r2pat=ieor(e2sub,ui)
778      continue
            call fetchitvar(indexes,fp,np,reset,r2pat(1:ntau),ntau,in1,in2,nthr)
      ...
      enddo
      call nextpatvar(misub,K,nord,iflag)
   enddo
...
return
end subroutine osd174var

subroutine mrbencodevar(me,codeword,g2,N,K)
...
! fast encoding for low-weight test patterns
  ...
  enddo
return
end subroutine mrbencodevar

subroutine nextpatvar(mi,k,iorder,iflag)
  integer*1 mi(k),ms(k)
