...
! -23 -1.3 CQ P28Z58W/77M  i3=4 n3=1
  if(msg37(1:3).eq.'CQ ') then
     ...
     if(i3.eq.4) then
        islash=index(msg37,'/')
        if(islash.le.0) then
           callsign=trim(msg37(ispc1+1:))
           nlencall=len_trim(callsign)
           if(nlencall.lt.1) return
           ispccall=index(callsign,' ')
! CQ 9D6T0GA4 Z space inside callsign of any length: protocol violation, or last char is digit
           if(ispccall.lt.nlencall .or. callsign(nlencall:nlencall).lt.':') then
              go to 900
           endif
        else
           if(islash.gt.ispc1 .and. islash.lt.ispc2) then
              if(islash-ispc1.le.ispc2-islash) then
                 callsign=msg37(islash+1:ispc2-1)
              else
                 callsign=msg37(ispc1+1:islash-1)
              endif
           endif
        endif
        ...
        if(islash.gt.0) then
           ...
           if(nlen.ge.10) then
              ...
              if(falsedec) go to 900
           else if(nlen.eq.6 .and. index(callsign,'/').lt.1) then
! 'CQ VK6ZRW'  protocol violation, i3 for stdcall shall be equal to 1
              do i=1,6
                 if(callsign(i:i).gt.'/' .and. callsign(i:i).lt.':') then
                    callmask6(i:i)='1'
                 else
                    callmask6(i:i)='0'
                 endif
              ...
              falsedec=.false.
              do i=1,3
                 if(callmask6.eq.mask6(i)) then
                    falsedec=.true.
                    exit
                 endif ! standard callsign, false decode
              enddo
              if(falsedec) go to 900
           endif
        endif
...
!  -7  0.2  400 ~ CQ LI9NAI/R CF97 * in some noisy setups such decodes possible with a high SNR
        ...
        if(msg37(ispc2-2:ispc2-1).eq.'/R' .or.                               &
             msg37(ispc2-2:ispc2-1).eq.'/P') then
           callsign=msg37(ispc1+1:ispc2-3)
        else
        ...
        if(ispc3-ispc2.eq.5 .and. msg37(ispc2+1:ispc2+1).gt.'@' .and.        &
             ...
             msg37(ispc2+4:ispc2+4).lt.':') grid=msg37(ispc2+1:ispc3-1)
        if(len_trim(grid).eq.4) then
           call chkgridvar(callsign,grid,lchkcall,lgvalid,lwrongcall)
           if(lwrongcall .or. .not.lgvalid) go to 900
        endif
     endif
...
! CQ Y09Q7/SS60M  i3=4 n3=1
        ...
        if(nlencall.eq.11 .and. islash.gt.0 .and. islash2.lt.1) then
           ...
           if(islash.lt.7) then
              if((callsign(islash+1:islash+1).lt.':' .and.                   &
                   callsign(islash+2:islash+2).lt.':') .or.                  &
                   callsign(islash+1:islash+1).eq.'Q' .or.                   &
                   callsign(nlencall:nlencall).lt.':') go to 900
           endif

           if(islash.gt.5) then
              if(callsign(islash-1:islash-1).lt.':' .or.                     &
                   (callsign(1:1).lt.':' .and. callsign(2:2).lt.':') .or.    &
                   callsign(1:1).eq.'Q') go to 900
           endif

           if(islash.gt.4 .and. islash.lt.8) then
              call_a=''
              call_b=''
              call_a=callsign(1:islash-1)
              call_b=callsign(islash+1:)
              falsedec=.false.
...
! CQ 039 P30KJE/P DR46 i3=2 n3=5
     if(msg37(4:4).lt.':' .and. msg37(4:4).gt.'/' .and. msg37(7:7).eq.' '     &
          .and. (i3.eq.1 .or. i3.eq.2)) then
        islash=index(msg37,'/')
        if(islash.gt.10 .and. islash.lt.15) then
           callsign=''
           grid=''
           callsign=msg37(8:islash-1)
           grid=msg37(islash+3:islash+6)
           ...
           falsedec=.false.
           call chkflscallvar('MYCALL      ',callsign,falsedec)
           if(falsedec) go to 900
        endif
     endif
     ...
     if(i3.eq.1 .and. iaptype.eq.0) then
        ...
        if(islash.gt.6) then
           ...
           if(len_trim(msg37).eq.ispc3-1) then
              ...
              return ! 4-word message, will be processed in ft8bvar.f90
           endif
        endif
        nlengrid=len_trim(grid)
        if(nlengrid.eq.4 .and. grid(1:1).gt.'@' .and. grid(4:4).lt.':') then
...
! ES6DO ST0VQA R174 invalid message, can not be transmitted
     ...
     if(indxr.gt.7) then
        ...
        if(ispc3-ispc2.eq.5) then
           if(msg37(ispc2+1:ispc2+1).gt.'@' .and.                            &
                msg37(ispc2+1:ispc2+1).lt.'S' .and.                          &
                msg37(ispc2+2:ispc2+2).gt.'@' .and.                          &
                msg37(ispc2+2:ispc2+2).lt.'S' .and.                          &
                msg37(ispc2+3:ispc2+3).lt.':' .and.                          &
                msg37(ispc2+4:ispc2+4).lt.':') then
              grid=msg37(ispc2+1:ispc3-1)
           else ! invalid message
              go to 900
           endif
...
! JH4PUL/3 JT1DN HB30 * ! non standard mycall, iaptype 41
  ...
  if(iaptype.eq.3 .or. iaptype.eq.11 .or. iaptype.eq.21 .or. iaptype.eq.41) then
     indxr=index(msg37,' R ')
     if(iaptype.eq.21 .and. indxr.gt.0) go to 900 ! invalid message in Hound mode
     islash1=index(msg37,'/')
     ispc1=index(msg37,' ')
     ispc2=index(msg37((ispc1+1):),' ')+ispc1
     if(ispc1.gt.3 .and. ispc2.gt.7) then
        if(islash1.lt.ispc1 .or. (islash1.eq.ispc2-2 .and.                   &
             msg37(ispc2-1:ispc2-1).lt.':' )) then ! AA1AAA/1 can be first or 2nd callsign
           if(msg37(1:ispc1-1).eq.mycall .and.                               &
                msg37(ispc1+1:ispc2-1).eq.hiscall) then
              ispc3=index(msg37((ispc2+1):),' ')+ispc2
              grid=''
              if(indxr.gt.7) then
                 ispc4=index(msg37((ispc3+1):),' ')+ispc3
                 if(ispc4-ispc3.eq.5 .and. msg37(ispc3+1:ispc3+1).gt.'@'     &
                      .and. msg37(ispc3+1:ispc3+1).lt.'S' .and.              &
                      msg37(ispc3+2:ispc3+2).gt.'@' .and.                    &
                      msg37(ispc3+2:ispc3+2).lt.'S' .and.                    &
                      msg37(ispc3+3:ispc3+3).lt.':' .and.                    &
                      msg37(ispc3+4:ispc3+4).lt.':') grid=msg37(ispc3+1:ispc4-1)
              else if(indxr.lt.1) then
                 if(ispc3-ispc2.eq.5 .and. msg37(ispc2+1:ispc2+1).gt.'@'     &
                      .and. msg37(ispc2+1:ispc2+1).lt.'S' .and.              &
                      msg37(ispc2+2:ispc2+2).gt.'@' .and.                    &
                      msg37(ispc2+2:ispc2+2).lt.'S' .and.                    &
                      msg37(ispc2+3:ispc2+3).lt.':' .and.                    &
                      msg37(ispc2+4:ispc2+4).lt.':') grid=msg37(ispc2+1:ispc3-1)
              ...
              if(len_trim(grid).eq.4 .and. grid.ne.hisgrid4) then
                 ! correctly decoded contest message
                 if(iaptype.eq.21) then
                    if(grid.eq.'RR73') then
                       ! AP mask 'mybcall hisbcall ???' can trigger RR73 message
                       return
                    else
                       go to 900
                    endif
                 endif
                 call chkgridvar(hiscall,grid,lchkcall,lgvalid,lwrongcall)
                 if(grid.eq.'RR73') then
                    lgvalid = .true.
                 else
                    if(.not.lgvalid) go to 900
                 endif
              endif
           endif
        endif
  ...
  if((i3.ge.1 .and. i3.le.3) .and. (index(msg37,' R ').gt.0 .or.             &
       index(msg37,'/R ').gt.0 .or. index(msg37,'/P ').gt.0)) then
...
! 'N40RTH AD2WPI/R MG62'
     ...
     if(islash1.gt.0 .and. ispc1.gt.3 .and. ispc2.gt.7) then
        ...
        if(islash1.gt.ispc1) then
           call_a=msg37(1:ispc1-1)
           call_b=msg37(ispc1+1:islash1-1)
        else
...
! check if grid is correct for call_2
  ...
  if(lcall1hash .and. i3.eq.1) then
     ! hash can be associated with a valid callsign in the table
     ispc1=index(msg37,' ')
...
! i3=0 n3=3 'CE3NTP 4C2DWN 5E ONE'
  ...
  if(iaptype.ne.2) then
     if(((i3.eq.1 .or. i3.eq.3) .and. index(msg37,' R ').le.0 .and.          &
          index(msg37,'/').le.0) .or. (i3.eq.0 .and. n3.eq.3)) then
        if(msg37(1:2).eq.'<.') go to 2
        ispc1=index(msg37,' ')
        ispc2=index(msg37((ispc1+1):),' ')+ispc1
        if(ispc1.gt.3 .and. ispc2.gt.7) then
           call_a=msg37(1:ispc1-1)
           if(call_a.eq.mycall) go to 2 ! exception to incoming calls
           call_b=msg37(ispc1+1:ispc2-1)
           include 'call_q.f90'
           falsedec=.false.
           call chkflscallvar(call_a,call_b,falsedec)
...
! 'R 5QCDOIY9 <...> RR73'
        if(index(trim(callsign),' ').gt.0 .or. (islash.lt.1 .and.            &
             callsign(nlencall:nlencall).lt.':')) go to 900
        include 'callsign_q.f90'
        falsedec=.false.
        call chklong8(callsign,falsedec)
...
! i3=4 n3=2 '<...> 6HGLDZ 5IN2 RRR'
        callsign=''
        nlenmsg=len_trim(msg37)
        if(msg37(1:2).eq.'<.') then
           if(msg37(nlenmsg-4:nlenmsg).eq.' RR73') then
              callsign=msg37(7:nlenmsg-5)
           else if(msg37(nlenmsg-2:nlenmsg).eq.' 73') then
              callsign=msg37(7:nlenmsg-3)
           else if(msg37(nlenmsg-3:nlenmsg).eq.' RRR') then
              callsign=msg37(7:nlenmsg-4)
           else
              callsign=msg37(7:nlenmsg)
           endif
        else
           ibracket=index(msg37,'<.')
           callsign=msg37(1:ibracket-2)
        endif
        nlencall=len_trim(callsign)
        if(nlencall.gt.0) then
           ispace=index(callsign(1:nlencall),' ')
           islash=index(callsign,'/')
           if(ispace.gt.0 .or. (islash.lt.1 .and.                         &
                callsign(nlencall:nlencall).lt.':') .or.                  &
                islash.eq.nlencall) go to 900
...
! KY2HTW/0O5M <...>  i3=4 n3=0
           if(islash.gt.4) then
              ...
              if(nlencall.eq.11 .and. islash2.lt.1) then
                 ...
                 if(islash.gt.5) then
                    if(callsign(islash-1:islash-1).lt.':' .or.            &
                         (callsign(1:1).lt.':' .and.callsign(2:2).lt.':') &
                         .or. callsign(1:1).eq.'Q') go to 900
                 endif
...
! 'J/.ZM1R-D1Q89'
     if(index(msg37,'/.').gt.0) go to 900
     decoded=msg37(1:22)
     falsedec=.false.
     call filtersfreevar(decoded,falsedec)
     if(falsedec) go to 900
  endif

! CQ_RLXA 551626 ^J65XI
  if(i3.eq.0 .and. (msg37(1:3).eq.'CQ_' .or. index(msg37,'^').gt.0)) go to 900
  go to 999
